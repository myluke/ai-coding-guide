...
> 这里有个容易混的点，官方专门点了：Proactive 提供的是比[自动模式](https://code.claude.com/docs/zh-CN/permission-modes)更强的「自主执行**指导**」，但它**无需更改你的权限模式**就能用——所以**工具运行前你照样会看到权限提示**。权限模式（第 20 篇讲的「实习生动手前问不问你」）管的是「要不要拦着问你」，Proactive 管的是「它的行事风格多激进」，两码事，别当成一个。
...
| **托管策略级** | [托管设置目录](https://code.claude.com/docs/zh-CN/settings)内的 `.claude/output-styles` | 由组织统一下发 |
